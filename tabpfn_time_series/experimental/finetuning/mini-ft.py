    ...
    for set_name, (set_X, set_y) in [
        ...
        for i in random_indices:
            ...
            try:
                ...
                logging.warning(
                    f"Error during evaluation prediction/metric calculation: {e}"
                )
                res = EvalResult(
                    mse=np.nan,
                    mae=np.nan,
...
def main():
    ...
    train_datasets_collection = reg.get_preprocessed_datasets(
        all_train_X, all_train_y, splitfn, max_data_size=10000
    )
    test_datasets_collection = reg.get_preprocessed_datasets(
        all_test_X, all_test_y, splitfn, max_data_size=10000
    )
    ...
    # Training loop
    train_dl = DataLoader(
        train_datasets_collection, batch_size=1, collate_fn=collate_for_tabpfn_dataset
    )
    test_dl = DataLoader(
        test_datasets_collection, batch_size=1, collate_fn=collate_for_tabpfn_dataset
    )
    ...
    for epoch in range(opt_config.n_epochs):
        ...
        for iter, data_batch in enumerate(
            tqdm(train_dl, desc=f"Epoch {epoch + 1}/{opt_config.n_epochs}")
        ):
            (
            ...
            if (iter + 1) % opt_config.gradient_accumulation_steps == 0 or (
                iter + 1
            ) == len(train_dl):
                optimizer.step()
                optimizer.zero_grad()
                print(
                    f" Loss in EPOCH {epoch + 1}, iter: {iter},"
        ...
        inf_loss_count = 0
        with torch.no_grad():
            for test_data_batch in tqdm(
                test_dl, desc=f"Epoch {epoch + 1}/{opt_config.n_epochs}"
            ):
                (
                    X_trains_preprocessed,
                    X_tests_preprocessed,
        ...
        logging.info(
            f"Test loss: {np.mean(test_loss)}, infinite loss count: {inf_loss_count}"
        )
