#!/usr/bin/env python
"""
Script to submit run_single_ft.py as an SBATCH job using submitit.
"""
...
SCRIPT_PATH = Path(__file__).parent / "run_single_ft.py"


def parse_args():
    # First get the base parser from run_single_ft.py
    ft_parser = common_parse_args(return_parser=True)

    # Create a new parser for submitit-specific arguments
    parser = argparse.ArgumentParser(
        description="Submit run_single_ft.py as an SBATCH job using submitit",
        parents=[ft_parser],  # Inherit all arguments from run_single_ft.py
...
class TrainingJob:
    ...
    def __call__(self):
        ...
        # Construct command to run run_single_ft.py
